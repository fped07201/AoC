...
def ExpandData(data):
    ...
    for coor in data:
        if coor["x1"]==coor["x2"]:
            ...
            x_sign = 1 if coor["x1"]<coor["x2"] else -1
            y_sign = 1 if coor["y1"]<coor["y2"] else -1
            x = list(range(coor["x1"], coor["x2"]+ x_sign, x_sign))
            y = list(range(coor["y1"], coor["y2"]+ y_sign, y_sign))
